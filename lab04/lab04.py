...
def balanced(m):
    ...
    """
    assert is_mobile(m)
    if is_mobile(m[1][2]) and not balanced(m[1][2]):
        return False
    elif is_mobile(m[2][2]) and not balanced(m[2][2]):
        return False
    else:
        if m[1][1]*total_weight(m[1][2])==m[2][1]*total_weight(m[2][2]):
            return True
        else:
            return False
...
def preorder(t):
    ...
    if is_leaf(t):
        ...
        digit=[]
        for fw in branches(t):
            digit=digit+preorder(fw)
        wzdsfw=[label(t)]+digit 
        return wzdsfw     
