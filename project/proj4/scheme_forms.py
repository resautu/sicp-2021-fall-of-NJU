...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        if signature.rest!=nil:
            judge=signature.rest
            while judge!=nil:
                if not scheme_symbolp(signature.rest.first):
                    raise SchemeError
                    return None
                judge=judge.rest
                
        if signature.rest==nil:
            lls_lam=LambdaProcedure(nil,expressions.rest,env)
        else:
            lls_lam=LambdaProcedure(signature.rest,expressions.rest,env)
        env.define(signature.first,lls_lam)
...
def do_and_form(expressions, env):
    ...
    if expressions==nil:
        ...
        lls_re=scheme_eval(expressions.first,env)
        if is_scheme_false(lls_re):
            return False
        return lls_re
...
def make_let_frame(bindings, env):
    ...
    while bindings!=nil:
        ...
        arg = scheme_eval(bindings.first.rest.first, env)
