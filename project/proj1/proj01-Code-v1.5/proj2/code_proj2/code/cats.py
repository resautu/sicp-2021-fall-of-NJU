...
def choose(paragraphs, select, k):
    ...
    if len(paragraphs)<=k:
        return ''
    elif len(paragraphs)>k and select(paragraphs[k]):
    ...
    if len(typed_words)==0 and len(reference_words)==0:
        ...
        while i<len(typed_words):
            ...
            if typed_words[i]==reference_words[i]:
                judge+=1
            i+=1
        return judge/len(typed_words)*100
    else:
        judge=0
        i=0
...
def sphinx_swap(start, goal, limit):
    ...
    def recui_1(start,goal):
        if not start or not goal:
            return abs(len(start)-len(goal))
        elif start[0]==goal[0]:
            return recui_1(start[1:],goal[1:])
        else:
            return 1+recui_1(start[1:],goal[1:])
    return recui_1(start,goal)
...
def feline_fixes(start, goal, limit):
    ...
    def lls(start, goal, count):
        if not start and not goal: # Fill in the condition
            ...
            return lls(start[1:],goal[1:],count)

        else:  
            add_diff = lls(start,goal[1:],count+1)# Fill in these lines
            remove_diff = lls(start[1:],goal,count+1) 
            substitute_diff = lls(start[1:],goal[1:],count+1)
            # BEGIN
            return min(add_diff,remove_diff,substitute_diff)
            # END
    return lls(start, goal, count=0)
...
def report_progress(typed, prompt, id, send):
    ...
    while i<min(len(typed),len(prompt)):
        ...
        if typed[i] == prompt[i]:
            ...
            break
        i+=1
    pro_gress=pro/len(prompt)
    dic={'id':id, 'progress':pro_gress}
    send(dic)
    return pro_gress
    ...
    for word in words:
        ...
        for player in players:
            ...
            if t<min_time:
                min_time=t
                min_player=player
        outcome[min_player] += [word_at(game, word)]
