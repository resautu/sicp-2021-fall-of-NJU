...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.can_contain(self) and place.ant.ant_contained==None:
...
class HarvesterAnt(Ant):
    """HarvesterAnt produces 1 additional food per turn for the colony."""             #q1
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        distance=0
        while not bee_place.is_hive:
            
            
            lls_bee=random_bee(bee_place.bees)
            if lls_bee!=None and distance>=self.min_range and distance<=self.max_range:
                return lls_bee
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health>amount:
            ...
            atk=amount+self.damage
        for bee in self.place.bees[:]:
            bee.reduce_health(atk)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.chew_countdown==0 and self.nearest_bee()!=None:
            self.eat(self.nearest_bee())
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while behind:
            if behind.ant:
                if not behind.ant.has_buffed and not behind.ant.is_container:
                    behind.ant.buff()
                    
                    
                elif behind.ant.is_container and behind.ant.ant_contained!=None:
                    if not behind.ant.has_buffed:
                        behind.ant.buff()
                    if not behind.ant.ant_contained.has_buffed:
                        behind.ant.ant_contained.buff()
                elif behind.ant.is_container and not behind.ant.has_buffed:
                    behind.ant.buff()
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        if target!=None and not target.scared:
            target.scare(2)
        # END Problem EC
